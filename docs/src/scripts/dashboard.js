...
d3.json("/SG1_Team6/data/dashboard_data.json").then((data) => {
    ...
    console.error("Error loading JSON. Make sure it's in the public/data folder.", error);
});
...
function renderAll() {
    ...
    renderDoubleBar("#chart1", globalData.by_house_type, "house_type");
    renderDoubleBar("#chart2", globalData.by_wealth, "wealth_level");
...
function renderDoubleBar(container, data, keyName) {
    if (!document.querySelector(container)) return;
    const totalWidth = document.querySelector(container).clientWidth || 500;
    ...
    const x = d3.scaleBand().domain(data.map(d => d[keyName].replace("_", " "))).range([0, width]).padding(0.2);
    const xSub = d3.scaleBand().domain(subgroups).range([0, x.bandwidth()]).padding(0.05);
    const y = d3.scaleLinear().domain([0, d3.max(data, d => Math.max(d.load_kw, d.solar_gen_kw)) * 1.1]).range([height, 0]);
...
function renderDonut(container, summaryData) {
    ...
    const radius = Math.min(totalWidth - legendWidth, totalHeight) / 2;
    ...
    const svg = d3.select(container).append("svg")
        ...
        .style("width", "100%")
        .style("height", "auto");
    const g = svg.append("g").attr("transform", `translate(${(totalWidth - legendWidth) / 2}, ${totalHeight / 2})`);
...
function renderDuckCurve(container, data, summary) {
    if (!document.querySelector(container)) return;
    const totalWidth = document.querySelector(container).clientWidth || 600;
    ...
    const y = d3.scaleLinear().domain([d3.min(data, d => d.net_load_kw) - 1, d3.max(data, d => d.load_kw) + 1]).range([height, 0]);
    ...
    g.append("g").attr("transform", `translate(0, ${height})`).call(d3.axisBottom(x).ticks(12).tickFormat(d => d + ":00"));
...
function renderBatteryArea(container, data) {
    ...
    const x = d3.scaleLinear().domain([0, 23]).range([0, width]);
    const y = d3.scaleLinear().domain([0, d3.max(data, d => d.battery_soc_kwh) * 1.2]).range([height, 0]);

    const area = d3.area().x(d => x(d.hour)).y0(height).y1(d => y(d.battery_soc_kwh)).curve(d3.curveMonotoneX);
...
function renderBubbleChart(container, data) {
    ...
    const x = d3.scaleLinear().domain([0, d3.max(data, d => d.grid_export_kw) * 1.1]).range([0, width]);
    const y = d3.scaleLinear().domain([0, d3.max(data, d => d.savings_dollars) * 1.1]).range([height, 0]);
...
function renderStackedChart(container, data) {
    ...
    const y = d3.scaleLinear().domain([0, d3.max(stackedData[stackedData.length - 1], d => d[1]) * 1.1]).range([height, 0]);
